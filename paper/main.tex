...
\author{%
    {[AUTHOR\_NAME\_PENDING]} \\
    {[AFFILIATION\_PENDING]} \\
    \texttt{[EMAIL\_PENDING]} \\[4pt]
...
\begin{equation}
    ...
    \label{eq:cusum}
\end{equation}
where $\mu_0$ is the running mean entropy and $\delta$ is a slack parameter. The routing decision is:
\begin{equation}
...
During DPO fine-tuning, we monitor the KL divergence between the policy $\pi_\theta$ and reference model $\pi_{\text{ref}}$ on a held-out canary set. If $D_{\text{KL}}(\pi_\theta \| \pi_{\text{ref}}) > 0.15$ nats at any epoch checkpoint, training is rolled back to the previous checkpoint. This prevents catastrophic forgetting while allowing meaningful policy updates.
...
Self-Consistency $k{=}5$ & 64.0 &  64 &  844.5 &  84{,}449 & 322.2 & 262.1 \\
\textbf{\metis{} Dynamic} & \textbf{96.0} & \textbf{96} & \textbf{53.4} & \textbf{5{,}338} & \textbf{51.2} & \textbf{104.3} \\
\bottomrule
\end{tabular}
\end{table}

\metis{} routes 48 requests via \textsc{Fast} ($<30$ tokens) and 52 via \textsc{Normal}/\textsc{Deep} (50--149 tokens), achieving a $4.3\times$ token reduction over forced CoT while gaining 5 percentage points of accuracy.

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{fig1_pareto_frontier.pdf}
\caption{Pareto frontier of accuracy vs.\ average token cost. \metis{} Dynamic Routing (star marker) dominates all baselines, occupying the upper-left region of optimal efficiency. Dashed iso-lines indicate constant accuracy-per-kilotokens ratios. Self-consistency $k{=}5$ pays $16\times$ the token cost of \metis{} for 32 fewer percentage points of accuracy, likely due to majority-vote degradation on heterogeneous question types.}
\label{fig:pareto}
\end{figure}
...
To verify that \metis{} routing generalizes beyond the mixed math/logic evaluation set, we evaluate on three established benchmarks: GSM8K (grade-school math), TruthfulQA (factual accuracy), and HumanEval (code generation). Each benchmark uses 50 samples evaluated via vLLM batched inference with both a baseline prompt and a \metis{}-style cognitive routing prompt.

\begin{table}[ht]
\centering
\caption{Generalization results across three benchmarks. Baseline uses a task-specific zero-shot prompt; \metis{} uses the cognitive routing system prompt with \texttt{<thinking>} tag elicitation. $\Delta$Acc.\ is the accuracy difference (positive = \metis{} better). $\Delta$Tok shows the token efficiency gain. HumanEval was unavailable in the evaluation environment.}
...
HumanEval              & \multicolumn{6}{c}{\emph{Dataset not available in evaluation environment}} \\
\bottomrule
\end{tabular}
\end{table}
...
The surprisingly low accuracy of self-consistency $k{=}5$ (64\%) on our mixed benchmark merits discussion. We attribute this to majority-vote failure on heterogeneous question types: when the 5 samples produce diverse answer formats (e.g., ``Au'' vs.\ ``Gold'' vs.\ ``The chemical symbol for gold is Au''), the vote fragments. This is a known failure mode~\citep{wang2022self} that \metis{} avoids by using a single, deterministic generation per query.
