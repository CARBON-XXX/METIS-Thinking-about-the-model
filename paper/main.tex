...
\begin{figure}[ht]
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}\textbf{[Placeholder]} P50 and P99 latency distributions under ShareGPT workload traces.\\\metis{} Dynamic vs.\ Vanilla vLLM at QPS $\in \{1, 2, 4, 8, 16\}$.\vspace{2cm}}}
\caption{Latency distribution comparison. Left: P50 (median) TTFT. Right: P99 (tail) TTFT. \metis{} routing reduces tail latency by reclaiming KV-cache capacity through early \textsc{Fast}-path termination.}
\label{fig:latency}
\end{figure}

\emph{Note: Table~\ref{tab:throughput} and Figure~\ref{fig:latency} are structural placeholders pending the ShareGPT stress test execution. Values will be populated upon completion of the system-level evaluation pipeline.}
